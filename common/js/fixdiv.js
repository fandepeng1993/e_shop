...
(function() {
    ...
    f = {
        ...
        hotTemplate: {
            ...
            tags: '<ol class="address_hot_abb" style="{$style}">{$text}</ol>', // tab标题
            tag: "<li><span {$className}>{$text}</span></li>",                 // tab标题内容
    ...
    anjiehotel.module.address = function(k) {
        ...
        function R() {
            ...
            for (b in f.hotData){
				s++;
				type_val += b;
				if(i%4==0 || b=='Z'){ // 能被4整除或者最后一个
					a.push(f.hotTemplate.tag.replaceWith({
						text: type_val,
						className: _.$s2t(k.hotSelected) == type_val ? "class=" + f.hotClassNames.tagSelected: ""
					}));
					type_val = "";
				}
				i++;
		...
		// tab内容填充 ul li
        function S() {
            var a = [],b,li_item="",type_val="";
			var i = 0;
		
			// b:tab头 hotData[b]:tab内容
            for (b in f.hotData) { // b:tab头标题
                var c = [],
				...
				if(i!=0){
					var item_len = c.split("</a>").length-1;
					li_item += "<div style='margin-left:5px; color:#005daa'>"+b+"</div>" + c;
					if(item_len%8 != 0){
						li_item += "<li style='float:none;'></li>";
					}
				}else{
					li_item += c;
				}
				...
				if(i%4==0 || b=='Z'){
					a.push(f.hotTemplate.items.replaceWith({
						text: li_item,
						display: $s2t(k.hotSelected) == $s2t(type_val) ? "": "style='display:none'",
						type: type_val
					})); // a:ul集合 加入ul
					li_item = "";
					type_val = "";
		...
		if(null == A || "" == A){
			A="@Beijing|北京|53@Shanghai|上海|321@Shenzhen|深圳|91@Guangzhou|广州|80@Qingdao|青岛|292@Chengdu|成都|324@Hangzhou|杭州|383@Wuhan|武汉|192@Tianjin|天津|343@Dalian|大连|248@Xiamen|厦门|61@Chongqing|重庆|394@"
		}	
...
(function() {
	var jsContainer = '<div id="tuna_alert" style="display:none;position:absolute;z-index:999;overflow:hidden;"></div><div id="tuna_jmpinfo" style="visibility:hidden;position:absolute;z-index:120;"></div>';
	var div_container = document.createElement("div"); 
	div_container.innerHTML = jsContainer;
	div_container.id = "jsContainer";
	div_container.className = "jsContainer";
