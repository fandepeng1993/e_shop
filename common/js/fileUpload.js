...
var uploadTools = {
    ...
    "initWithCleanFile":function(opt){
        ...
        if(!opt.isHiddenCleanBt){
            ...
            $("#"+uploadId+" .uploadBts .cleanFileBt i").css("color","");
        }
    },
    /**
     * 初始化选择文件按钮
    ...
    "initWithSelectFile":function(opt){
        var uploadId = opt.uploadId;
        $("#"+uploadId+" .uploadBts .selectFileBt").css("background-color","");
    ...
    "getShowFileType":function(isImg,fileType,fileName,isImgUrl,fileCodeId){
        var showTypeStr="<div class='fileType'>"+fileType+"</div> <i class='iconfont icon-iconwenjianjia'></i>";//默认显示类型
        if(isImg){
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<div class='selectFileBt'><span class='iconfont icon-shangchuantupian'></span></div>";
        btsStr += "</div>";
        //上传按钮
        if(!opt.isHiddenUploadBt){
            btsStr += "<div class='uploadFileBt'>";
            btsStr += "<i class='iconfont icon-iconshangchuan'></i>";
            btsStr += " </div>";
        }
        //清理按钮
        if(!opt.isHiddenCleanBt){
            btsStr += "<div class='cleanFileBt'>";
            btsStr += "<i class='iconfont icon-iconfontqingchu'></i>";
            btsStr += " </div>";
        }
        btsStr += "</div>";
        ...
        for(var i=0;i<fileListArray.length;i++){
            ...
            if(testbytesRead<0){
                if(percent==100){
                    ...
                    $("#"+uploadId+" .box .fileItem[fileCodeId='"+i+"'] .status>i").addClass("iconfont icon-guanbi");
                ...
                $("#"+uploadId+" .box .fileItem[fileCodeId='"+i+"'] .status>i").addClass("iconfont icon-icongou");
                if(opt.showFileItemProgress) {
                    $("#" + uploadId + " .box .fileItem[fileCodeId='" + i + "'] .progress>div").css("width", "100%");
                }
                bytesRead = bytesRead-fileListArray[i].size;
    ...
    "uploadError":function(opt){
        ...
        $("#"+uploadId+" .box .fileItem .status>i").addClass("iconfont icon-guanbi");
        var progressBar = $("#"+uploadId+" .subberProgress .progress>div");
    ...
    "disableFileUpload":function(opt){
        if(!opt.isHiddenUploadBt){
            var uploadId = opt.uploadId;
            $("#"+uploadId+" .uploadBts .uploadFileBt").off();
            $("#"+uploadId+" .uploadBts .uploadFileBt i").css("color","");
        }
    },
    /**
     * 禁用文件清除
